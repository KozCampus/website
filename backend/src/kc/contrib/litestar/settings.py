...
from kc.contrib.redis.settings import RedisSettings
from kc.contrib.sqlalchemy.settings import SQLAlchemySettings
...
class APISettings(Struct):
    app_name: str = "KözCampus API"
    """
    Application name displayed e.g. in the OpenAPI documentation.
    """

    debug: bool = os.environ.get("KC_DEBUG", "True") in TRUE_VALUES
    """
    ...
    cache_expiration: int = int(os.environ.get("KC_CACHE_EXPIRATION", 60))
    """
...
class SSOProviderSettings(Struct):
    client_id: str = os.environ.get("KC_GOOGLE_CLIENT_ID", "")
    """
    OAuth2 client ID assigned by the provider.
    """

    client_secret: str = os.environ.get("KC_GOOGLE_CLIENT_SECRET", "")
    """
