...
from kc.contrib.sqlalchemy.engine import create_engine


class SQLAlchemyEngineSettings(Struct):
    url: str = os.environ.get(
        "KC_SQLALCHEMY_URL",
        "postgresql+asyncpg://kozcampus:kozcampus@localhost:15432/kozcampus",
    )
    """
...
class SQLAlchemySettings(SQLAlchemyEngineSettings):
    ...
    db: SQLAlchemyEngineSettings = msgspec.field(
        default_factory=SQLAlchemyEngineSettings,
    )
...
def create_sqlalchemy_config(
    ...
    return SQLAlchemyAsyncConfig(
        engine_instance=create_engine(settings.db),
